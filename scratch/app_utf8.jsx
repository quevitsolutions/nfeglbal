...
const NAV_ITEMS = [
  { id: 'earn',      icon: 'Γ¢Å∩╕Å',  label: 'Mine' },
  { id: 'mine',      icon: '≡ƒÜÇ',  label: 'Boost' },
  { id: 'friends',   icon: '≡ƒæÑ',  label: 'Friends' },
  { id: 'team',      icon: '≡ƒîÉ',  label: 'Team' },
  { id: 'dash',      icon: '≡ƒôè',  label: 'Stats' },
  { id: 'contracts', icon: '≡ƒôä',  label: 'Docs' },
  { id: 'tasks',     icon: 'Γ£à',  label: 'Tasks' },
  { id: 'events',    icon: '≡ƒîÇ',  label: 'MetaVerse' },
];
...
function DesktopSidebar({ activeTab, setActiveTab, nodeId, nodeTier, isAdmin, hasNode }) {
  ...
  if (isAdmin) {
    tabs.push({ id: 'admin', icon: 'ΓÜí', label: 'Master Admin' });
  }
...
export default function App() {
  ...
  // Real-time chain event listener ΓÇö pushes DB updates + toast notifications
  useChainEvents();
  ...
  useEffect(() => {
    ...
    if (ref && /^(0x[a-fA-F0-9]{40}|\d+)$/i.test(ref)) {
      setReferrerId(ref);
      // Persist to localStorage immediately ΓÇö survives MetaMask redirect & page reload
  ...
  useEffect(() => {
    ...
    if (sponsorWallet) {
      setTimeout(() => {
        toast(
          `≡ƒñ¥ Referred by ${shortAddr(sponsorWallet)} ΓÇö Welcome to AIPCore!`,
          {
            duration: 6000,
            icon: '≡ƒöù',
            style: {
      ...
      setTimeout(() => {
        toast(
          '≡ƒÜÇ Welcome to AIPCore! Start mining and invite friends to earn more.',
          { duration: 5000, icon: 'Γ¼í', style: { background: 'rgba(203,255,1,0.1)', border: '1px solid rgba(203,255,1,0.3)', color: '#fff', fontWeight: 800, fontSize: 13 } }
        );
      }, 1500);
    }
  ...
  return (
    <div className="app-container">
      ...
      {/* TopBar ΓÇö fixed on mobile/tablet, grid on desktop */}
      ...
      {/* Bottom tabs ΓÇö hidden on desktop via CSS */}
      <TabBar />
