...
const SESSIONS = [
  {
    ...
    host: 'AIPCore AI Host', hostIcon: '🤖', viewers: 1240, duration: '1h 24m',
  ...
  },
  {
    ...
    host: 'AIPCore Academy', hostIcon: '🎓', viewers: 0, duration: '~45m',
  ...
  },
  {
    ...
    host: 'AIPCore AI Host', hostIcon: '🤖', viewers: 4820, duration: '58m',
...
export default function AIPCoreLiveScreen({ onBack, onEnterHall }) {
  ...
  const handleJoin = () => {
    ...
    setJoined(true);
    toast.success('🎙️ Joined live session! You\'ll earn 500 $AIPCORE on completion.', { duration: 5000 });
  };
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18 }}>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <h1 style={{ fontSize: 18, fontWeight: 900 }}>🎙️ AIPCore Live</h1>
        ...
        {SESSIONS.filter(s => s.status === tab || (tab === 'live' && s.status === 'live')).map(s => (
          <motion.div key={s.id} whileHover={{ scale: 1.01 }}
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              ...
              {s.reward > 0 && <span style={{ background: 'rgba(163,255,18,0.1)', color: '#A3FF12', fontSize: 9, fontWeight: 900, padding: '3px 8px', borderRadius: 8 }}>+{s.reward} $AIPCORE</span>}
              ...
              {!joined ? (
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:10, alignItems:'center' }}>
                  <div style={{ background: 'rgba(163,255,18,0.1)', border: '1px solid rgba(163,255,18,0.3)', borderRadius: 10, padding: '8px 16px', fontSize: 12, fontWeight: 800, color: '#A3FF12' }}>
                    ✅ You are live! Earn {session.reward} $AIPCORE on completion
                  </div>
                  <button onClick={handleEnterHall}
      ...
      {session && session.status === 'upcoming' && (
        <div style={{ background: 'rgba(22,27,34,0.9)', border: '1px solid rgba(255,215,0,0.2)', borderRadius: 18, padding: 20, textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'center', marginBottom: 16 }}>
            <span style={{ background: 'rgba(163,255,18,0.1)', color: '#A3FF12', fontSize: 11, fontWeight: 900, padding: '5px 12px', borderRadius: 10 }}>+{session.reward} $AIPCORE</span>
